...
function getAllProjectIds() {
  return projectList.map((item: any) => item.id);
}
...
// 获取指定项目的所有版本号
function getAllVersionsOfProject(project: string) {
  return projectVersions[project]
    ? projectVersions[project].map((v: any) => v.version)
    : [];
}

// 检查版本号是否有效
function isValidVersion(project: string, version: string): boolean {
